...
function AddEventHandler(EventName, Handler, RunOnce)
	event(EventName):register(function(e, ...) if RunOnce then e:removeThisCallback() end return Handler(...) end)
	if not _G[EventName] then _G[EventName] = function(...) return event(EventName)(...) end
	end
end