		...
		if (style != SCE_ERR_DEFAULT &&
			...
			if (sourceLine >= 0) {
				...
				if (props.GetInt("lexer.errorlist.findliststyle", 1) && '.' == source[0] && pathSepChar == source[1]) {
					...
					std::string topic;
					while (topLine >= 0) {
						int startPos = wOutput.PositionFromLine(topLine); //wOutput.Call(SCI_POSITIONFROMLINE, topLine, 0);
						int lineLen = wOutput.LineLength(topLine); //wOutput.Call(SCI_LINELENGTH, topLine, 0);
