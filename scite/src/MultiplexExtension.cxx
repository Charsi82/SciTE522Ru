...
//!-start-[OnSendEditor]
const char* MultiplexExtension::OnSendEditor(Scintilla::Message msg, unsigned int wp, const char* lp) {
	const char* result = 0;
	for (Extension* pexp : extensions) {
...
const char* MultiplexExtension::OnSendEditor(Scintilla::Message msg, unsigned int wp, long lp) {
	const char* result = 0;
	for (Extension* pexp : extensions) {
		result = pexp->OnSendEditor(msg, wp, lp);
		if (result != 0) break;
